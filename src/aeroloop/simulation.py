...
def record(result, output_root):
    run_id = f"cpu-{result['config']['scenario']}-{result['config']['seed']}-{uuid.uuid4().hex[:12]}"
    ...
    manifest = {"schema_version": 1, "run_id": run_id, "kind": "recorded_simulation", "fixture": False,
                "captured_at": datetime.now(timezone.utc).isoformat(), "experiment": "cpu-rigid-body",
                "model": "ideal-body-wrench-v1", "controller": "rate-pid-v1", "scenario": result["config"]["scenario"],
