...
export function evidenceBase(base: string, origin: string): URL {
  ...
  if (
    ...
    throw Error("Evidence requires a same-origin directory");
  return url;
}
...
export async function loadIndex(base: URL, hash: string, signal: AbortSignal) {
  return validateIndex(
    await readVerified(new URL("index.json", base), hash, signal, 65536),
  );
}
export async function loadRecording(
  ...
  const values = await Promise.all(
    ["replay", "manifest", "metrics", "events"].map((name) => {
      const path = `${entry.run_id}/${name}.json`;
      return readVerified(
        new URL(path, base),
        index.checksums[path],
        signal,
        name === "replay" ? 4 * 1024 * 1024 : 65536,
      );
    }),
  );
  return validateRecording(
    entry,
    ...(values as [unknown, unknown, unknown, unknown]),
  );
}
